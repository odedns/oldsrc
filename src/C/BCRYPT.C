...
#if defined(_MICROSOFT) || defined(__TURBOC__)
...
{2
    ...
    iobuf[0]=salt[0];
    iobuf[1]=salt[1];
    bcrypt_set_word(plaintext);
    bcrypt_salt_to_E(salt[0], salt[1], SL);
    b=bcrypt_encode(SL[0], SL[1]);
    b.L=DE(b.L);
    b.R=DE(b.R);
    b=FP(b);
    bcrypt_BU64_to_pw(b, iobuf+2);
    return(iobuf);
}
#endif
...
#ifdef KC_BCRYPT
BU64 kc_bcrypt(dpw, SL0, SL1, nword)
    char *dpw;
    U32 SL0, SL1;
    int nword;
{
