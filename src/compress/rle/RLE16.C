 ...
 * max sequence is 32767 bytes of either repeating
 * or non-repeating stuff.
 * (uses 16 bit headers.)
 *
 * This program and its source are strictly public domain
 *
 * Written by Shaun Case, 1991, Borland C++ 2.0
 *
 * doesn't do error checking on fputc()
 */
...
int main(int argc, char **argv)
{
    ...
    strcat(outfile_name, ".r16");

    if (argc != 2)
    {
        puts("Usage: RLE16 filename.ext compresses file to filename.r16");
        return 1;
    }

    puts("rle16  by Shaun Case 1991  public domain.");
    ...
    while (!feof(infile))
    {
        ...
        if (seq_len ==0)                /* haven't got a sequence yet   */
        {
            ...
            if (run_char == cur_char)   /* got another char in the run  */
                if (++run_len == MAX_LEN)
                {
                    fputc((int)MAX_RUN_HEADER & (0xff), outfile);   /* write lo byte   */
                    fputc((int)MAX_RUN_HEADER >> 8, outfile);       /* write high byte */
            ...
            if (run_len > 3)       /* so write out the run and      */
                                   /* start a new one of the new    */
                                   /* character.                    */
            {
                fputc((int)(run_len & 0xff), outfile);      /* lo byte */
                fputc((int)(RUN | run_len) >> 8, outfile);  /* hi byte */
                fputc((int)run_char, outfile);
            ...
            for (j = 0; j < run_len; j++);    /* copy 1 or 2 char run to seq[]   */
            {
                ...
                {
                    fputc((int)MAX_SEQ_HEADER & 0xff, outfile);   /* lo byte */
                    fputc((int)(MAX_SEQ_HEADER >> 8) , outfile);   /* lo byte */
            ...
            if (run_len != 0)           /* if a run exists */
            {
                if (cur_char == run_char )  /* add to run!  Yay.  */
                {
                    ...
                    if (run_len == MAX_LEN)  /* if run is full */
                    {
                        ...
                        fputc((int)seq_len & 0xff, outfile);          /* lo byte */
                        fputc((int)((SEQ | seq_len) >> 8), outfile);   /* lo byte */
                ...
                fputc((int)seq_len & 0xff, outfile);          /* lo byte */
                fputc((int)((SEQ | seq_len) >> 8), outfile);   /* hi byte */
