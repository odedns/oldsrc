...
function openTree(nodeSign,isResize)
{
	...
	var node = getObjectById("row" + nodeId);
	...
	{
		modelId = nodeId.substring(0,nodeId.indexOf("-"));
	}
	var currentLevel;
	if (node.hasSons == "true") 
	{
		...
		if (isOpenAction && !isRoot)
		{
			...
			{
				document.all(node.id.substring(node.id.indexOf("row") + 3)).scrollIntoView();
			} 
			else if ((node.offsetTop + node.offsetHeight) > (treeDiv.clientHeight + treeDiv.scrollTop))
			{
				document.all(node.id.substring(node.id.indexOf("row") + 3)).scrollIntoView(false);
			} 						
		}
...
function isParent(nodeId,parentNodeId)
{
	...
	if (idx != -1) 
	{
		if (nodeId.charAt(idx + parentNodeId.length) == "-") return true;
	}
	return false;
}
...
{
	if (nodeSign.children(0).tagName == "IMG") {
...
/*save tree status*/
function saveTreeState(modelId)
{
	var form = document.forms[0];
	if (form != null) {
		var rows = document.all(modelId + "Body").rows;
		var fields = "";
		var l = rows.length;
		for (index = 0;index < l;index++) {
			var row = rows[index];	
			if (row.hasSons == "true") {
				if (row.showSons == "true") {
					fields += row.rid;
					if (index != l - 1) {
						fields += seperator;
					}				
		...
		if (form.elements["TO" + modelId] == null) {
			form.insertAdjacentHTML("AfterBegin",getHiddenHtml("TO" + modelId,fields));
		}
		else {
			form.elements["TO" + modelId].value = fields;
		}
	}
}
