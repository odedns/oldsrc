...
{
	if (obj != null)
		obj.style.display = HTML_VALUE_NONE;
}
...
{
	obj.style.visibility = HTML_VALUE_VISIBLE;	
}

function setHidden(obj)
{
	obj.style.visibility = HTML_VALUE_HIDDEN;	
}

function isDisplayed(obj)
{
	return obj.style.display != HTML_VALUE_NONE;
}

function isVisible(obj)
{
	return obj.style.visibility != HTML_VALUE_HIDDEN;
}
 ...
 */
function createNewFormByForm(form,submitNewForm)
{
...
/**
 * Function that creates a new form and 
 * parameters : 
 * name
 * action
 * method
 ...
 * submitNewForm - boolean parameter,if true the newely created form is submitted.	
 */
function createNewForm(name,action,method,target,params,submitNewForm,isDialog,dialogParams)
{
	var newForm = document.createElement("form");	
	...
	document.appendChild(newForm);
 	...
 	var paramNames = params.getKeys();
 	
 	for (index = 0;index < paramSize;index++)
 	{
 		var paramName = paramNames[index];
 		newForm.appendChild(getHidden(paramName,params.get(paramName)));
  	}
  	
 	//Submits new form if submitNewForm=true
	if (submitNewForm)
	{
		//Opens new form in a modal dialog 
		if (isDialog)
		{
		...
		//Opens new form in popup window
		else
		{
			if (dialogParams == null || dialogParams == "" || dialogParams == " ")
			{
			...
			window.open('',newForm.target,HELP_POPUP_DEFAULT_PARAMS);
			newForm.submit();
			document.removeChild(newForm);
		}
	}
