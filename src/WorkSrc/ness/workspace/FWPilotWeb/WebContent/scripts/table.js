...
function createMenu(openMark,modelId,rowId,menuStr,menuClassName)
{
	...
	if (countChecked(document.all(modelId + "Checkbox")) < 2)
	{
		...
		floatingMenuPopup.show( (document.body.dir == "rtl" ? -menu.offsetWidth + relativeObject.offsetWidth : 0) ,0,menu.offsetWidth,menu.offsetHeight + 2,relativeObject);
...
{
	var menuItemsProps = tokenizer(menuItemStr,"-");
	var index = menuItemStr.indexOf("-");
	var lastIndex = menuItemStr.lastIndexOf("-");
...
{
	if (document.all("SelectAll" + modelId) != null) 
	{
		var modelCheckboxes = getObjectById(modelId + "Checkbox");
		if (modelCheckboxes != null) {
			if (modelCheckboxes.length == null) 
			{
				document.all("SelectAll" + modelId).checked = modelCheckboxes.checked;
			}
			else 
			{
				var l = modelCheckboxes.length;
				for (index = 0;index < l;index++) {
					...
					{
						document.all("SelectAll" + modelId).checked = false;
						return;			
					}
				}
				document.all("SelectAll" + modelId).checked = true;
			}
		}
	}
}
...
{
	var startIndex = extraData.indexOf(paramName + "-");
	if (startIndex != -1)
	{
...
{
	if (self[tableModelId + "Columns"] != null && self[tableModelId + "Columns"].length > 0)
	{
		...
		//sortPopup.show(0,0,POPUP_SORT_WIDTH,POPUP_SORT_ORDER_HEIGHT,relativeObject);
		sortPopup.show((document.body.dir == "ltr" ? - POPUP_SORT_WIDTH + relativeObject.offsetWidth : 0) , 0,POPUP_SORT_WIDTH,POPUP_SORT_ORDER_HEIGHT,relativeObject);
		sortPopup.document.all.initButton.fireEvent("onclick");	
	}
}
...
{
	if (self[tableModelId + "ColumnsOrder"] != null && self[tableModelId + "ColumnsOrder"].length > 0)
	{
		...
		columnOrderPopup.show((document.body.dir == "ltr" ? - POPUP_COLUMN_ORDER_WIDTH + relativeObject.offsetWidth : 0) , 0,POPUP_COLUMN_ORDER_WIDTH,POPUP_COLUMN_ORDER_HEIGHT,relativeObject);
		//columnOrderPopup.show(0,0,POPUP_COLUMN_ORDER_WIDTH,POPUP_COLUMN_ORDER_HEIGHT,relativeObject);
		columnOrderPopup.document.all.initButton.fireEvent("onclick");	
	}	
}
