...
<%@ page import="onjlib.servlet.*" %>

<p> Weblogic session in request: <%=request.getParameter("WebLogicSession") %>
<%
  HttpSession session1 = GSessionManager.getInstance().getSession(request);
  if (request.getParameter("AddValue") != null) {
  	session1.setAttribute("foo1","bar1");
    session1.setAttribute(addPrefix(request.getParameter("NameField")), 
  	              request.getParameter("ValueField"));
  } else if (request.getParameter("DeleteValue") != null) {
    session1.removeAttribute(addPrefix(request.getParameter("NameField")));
  }
%>

<p> session id: <%= session1.getId() %>
...
<%
  Enumeration sessionNames = session1.getAttributeNames();
  String name;
...
<td><%= (String)session1.getAttribute(name) %></td>
</tr>
...
<form method="post" name="SessionServlet" action=
<%=response.encodeRedirectURL("/servlet/genhttpservlet?next_page=/SessionServlet.jsp") %>
>

<p> URL <%= response.encodeRedirectURL("/servlet/genhttpservlet?next_page=/SessionServlet.jsp") %>
<center>
