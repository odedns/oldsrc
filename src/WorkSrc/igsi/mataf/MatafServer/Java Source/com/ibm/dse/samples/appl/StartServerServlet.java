package com.ibm.dse.samples.appl;
 ...
 * @copyright(c) Copyright IBM Corporation 1999, 2003.
 */
public class StartServerServlet extends javax.servlet.http.HttpServlet implements CSInactivityClientListener,CSProcessorInactivityListener, CSReplyErrorEventListener{
...
/**
 * Converts all blanks in the String to a key instead of a 
 * space. The key is "dse_Blank". This method is used if the path passed as a parameter 
 * in the main method (-p parameter) has spaces, so the path must not contain the key.
...
/**
 * Handles the events that the CSServer
 * will throw when the response cannot be sent to the Client 
 * due to a failure of the connection.
 * @param event com.ibm.dse.cs.servlet.CSReplyErrorEvent
 */
	
public void handleCSReplyErrorEvent(CSReplyErrorEvent event){
	
	System.out.println(resResources.getString("Reply_Error_Message"));
	System.out.println(resResources.getString("Reply_Error_Info"));
	System.out.println(resResources.getString("Reply_Error") + event.getReplyError());
	System.out.println(resResources.getString("Formated_Data") + event.getFormattedData());
	System.out.println(resResources.getString("Reply_Format_Name") + event.getReplyFormatName());
	System.out.println(resResources.getString("Detail_Error") + event.getDetailError());
 ...
 *		-s <address>	IP address where the servlet's server is running<br>
...
/**
 * Initializes the servlet. To avoid the toolkit initialization when the Application Server is initiated, set the
...
protected void initialize() throws Exception 
{
	...
	{
		if (iniPath != null) Settings.reset(iniPath);
		else				 Settings.reset("c:\\dse\\server\\dse.ini");//$NON-NLS-1$
	}
	...
	//Add this object as the listener for the error reply event
	((CSServerService)context.getService("CSServer")).addCSReplyErrorEventListener(this);
}
/**
 * Creates an initial context in the server and starts the client server service.
...
public static void main(String args[]) {
	...
	try {
		...
		URL myURL = null;
		if (path != null)
			myURL=new URL("http://"+ server + "/servlet/com.ibm.dse.samples.appl.StartServerServlet?" + path);//$NON-NLS-2$//$NON-NLS-1$
		else
			myURL=new URL("http://"+ server + "/servlet/com.ibm.dse.samples.appl.StartServerServlet");//$NON-NLS-2$//$NON-NLS-1$
 ...
 * on this paramether. This method is common for any client type (HTML, java, and so on).
...
{
	String message=resResources.getString("Initialization_OK"); //$NON-NLS-1$
...
/**
 * Utility method that looks in the mainString for all occurrences of the 
