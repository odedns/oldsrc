...
/**
 * Closes the journal access.
 */
public void closeJournal() throws Exception {
	JournalService journal;
	/* Does nothing if the journal service is not available */
	journal = (JournalService)getService("dummyJournal");//$NON-NLS-1$
	if (journal == null) return;
	journal.disconnect();
	journal.close();
}
/**
 * Closes the electronic journal and removes the session entry in the
